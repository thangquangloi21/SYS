...
class TabViewApp:
    ...
    def __init__(self, root):
        ...
        if not self.is_first_instance():
            try:
                ...
                # return  # Đã comment, không cần thiết vì destroy sẽ thoát
    ...
    def create_tab3(self):
        ...
        # texbox
        # entry = tk.Entry(tab2, width=15)
        texboxsl = Entry(self.tab3, width = 5)
        texboxsl.grid(row=0, column=1, padx=10, pady=5)
        #checkbox
        Intermediate = IntVar() 
        Intermediatecheck = Checkbutton(self.tab3, text = "add Intermediate", 
            ...
            while True:
                ...
                if meslogin in windows:
                    ...
                    while True:
                        ...
                        status = self.timanh08(r".\img\anhchuatich.png")
                        if not status:
    ...
    def checkopenapp(self,title, Path):
        try:
            ...
            if self.is_app_open('RemoteApp security warning'):
                print("Doi ti dang ok")
            else:
